...
Download the pomodoro-timer.html file
...
Choose a Mode: Click on "Work", "Short Break", or "Long Break" buttons
Start Timer: Click the play button (▶) in the center
Pause: Click the pause button (⏸) to pause the timer
Reset: Click the reset button (↻) to reset to the current mode's duration
Skip: Click the skip button (⏭) to move to the next session

Customizing Durations

Enter your desired minutes in the input fields:
...
Click the "Apply Settings" button to update the timer
The timer will reset to the new duration for the current mode
...
html<input type="number" id="workInput" class="form-control" value="25" min="1" max="60" />
